...
export function Navigation({ searchQuery, onSearchChange, onNavigateToSettings, onNavigateToFavorites, contentTypeFilter, onContentTypeChange }: NavigationProps) {
    ...
    <nav className="sticky top-0 z-50 bg-primary border-b border-primary-foreground/10">
            ...
            <div className="relative">
              ...
              {isProfileOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-3 w-80 min-w-[320px] bg-white border border-gray-200 rounded-2xl shadow-xl z-50 overflow-hidden">
                    <div className="p-5 border-b border-gray-100">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="font-bold text-gray-900 truncate">{user?.name || user?.email || 'Usuário'}</p>
                          <p className="text-sm text-gray-500 break-words">{user?.email || 'Consultor Pedagógico'}</p>
                        </div>
                      </div>
                    </div>
      ...
      {isProfileOpen && (
        <>
          ...
          <div className="lg:hidden absolute top-full right-4 mt-2 w-[calc(100vw-2rem)] max-w-sm min-w-[320px] bg-white border border-gray-200 rounded-2xl shadow-xl z-50 overflow-hidden">
            <div className="p-5 border-b border-gray-100">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-primary rounded-xl flex items-center justify-center flex-shrink-0">
                  <User className="w-6 h-6 text-white" />
